...
describe('ExtraUser Management Update Component', () => {
  ...
  let extraUserFormService: ExtraUserFormService;
  ...
  beforeEach(() => {
    ...
    extraUserFormService = TestBed.inject(ExtraUserFormService);
  ...
  describe('ngOnInit', () => {
    ...
    it('Should update editForm', () => {
      ...
      expect(comp.extraUser).toEqual(extraUser);
    });
  });
  ...
  describe('save', () => {
    it('Should call update service on save for existing entity', () => {
      // GIVEN
      const saveSubject = new Subject<HttpResponse<IExtraUser>>();
      const extraUser = { id: 123 };
      jest.spyOn(extraUserFormService, 'getExtraUser').mockReturnValue(extraUser);
      ...
      // THEN
      expect(extraUserFormService.getExtraUser).toHaveBeenCalled();
      expect(comp.previousState).toHaveBeenCalled();
      expect(extraUserService.update).toHaveBeenCalledWith(expect.objectContaining(extraUser));
    ...
    it('Should call create service on save for new entity', () => {
      ...
      const saveSubject = new Subject<HttpResponse<IExtraUser>>();
      const extraUser = { id: 123 };
      jest.spyOn(extraUserFormService, 'getExtraUser').mockReturnValue({ id: null });
      jest.spyOn(extraUserService, 'create').mockReturnValue(saveSubject);
      jest.spyOn(comp, 'previousState');
      activatedRoute.data = of({ extraUser: null });
      comp.ngOnInit();
      ...
      // THEN
      expect(extraUserFormService.getExtraUser).toHaveBeenCalled();
      expect(extraUserService.create).toHaveBeenCalled();
      expect(comp.isSaving).toEqual(false);
      expect(comp.previousState).toHaveBeenCalled();
    });
    ...
    it('Should set isSaving to false on error', () => {
      ...
      // THEN
      expect(extraUserService.update).toHaveBeenCalled();
      expect(comp.isSaving).toEqual(false);
